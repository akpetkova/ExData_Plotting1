...
#         unzip("./data/exdata-data-household_power_consumption.zip")
        ...
        png(filename="./data/plot4.png", width = 480, height = 480, units="px", bg="transparent")
        par(mfcol=c(2,2))
        
        # Generate the "Global Active Power" plot
        plot(data$datetime, data$globalactivepower, type="l", ylab = "Global Active Power", xlab = "", cex.lab=0.8, cex.axis=0.8)
        
        # Generate the multiple-line plot       
        plot(data$datetime, data$submetering1, type="l", ylab = "Energy sub metering", xlab = "", col="black",cex.lab=0.8)
        lines(data$datetime, data$submetering2, type="l", xlab = "", col="red")
        lines(data$datetime, data$submetering3, type="l", xlab = "", col="blue")
        legend("topright", legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=1, col=c("black", "red", "blue"), cex=0.8, xjust=0)
        par(bg="transparent", cex=1)  
        
        # Generate the "Voltage" plot
        plot(data$datetime, data$voltage, type="l", ylab = "Voltage", xlab = "datetime", cex.lab=0.8, cex.axis=0.8)
        
        # Generate the "Global reActive Power" plot
        plot(data$datetime, data$globalreactivepower, type="l", ylab = "Global_rective_power", xlab = "datetime", cex.lab=0.8, cex.axis=0.8)
